...
void handle_pseudo_cmd(FILE *file, int lineNum, int cmdNum, char *pOpcode, char *pArg1, char *pArg2,
	...
	if (!strncmp(pOpcode, "la", 2) && strlen(pOpcode) == 2) {
		...
		int i = handle_label_imm(pArg2, pLabel, cmdNum, lineNum);
		int code = 0;
		code = (0x0D << 2) + 0x03;
...
int cmd2code(int lineNum, int cmdNum, char *pOpcode, char *pArg1, char *pArg2,
	...
	if (!strncmp(pOpcode, "addi", 4) && strlen(pOpcode) == 4) {
		...
		code = (0x0D << 2) + 0x03;
		code += (reg2num(pArg1, lineNum) << 7);
		code += (0xFFFFF000 & (validate_imm(pArg3, 30, lineNum))); //mask 20 bits value
	...
	// Unconditional Jumps
		...
		code = (0x19 << 2) + 3;
		code += (reg2num(pArg1, lineNum) << 7);
		code += (reg2num(parse_parenthesized_expression(pArg2, lineNum)->reg, lineNum) << 15);
		code += (MASK11_0(parse_parenthesized_expression(pArg2, lineNum)->imm) << 20);
	...
	// Load and Store Instructions
		...
		code += (reg2num(parse_parenthesized_expression(pArg2, lineNum)->reg, lineNum) << 15);
		int offset = (MASK11_0(parse_parenthesized_expression(pArg2, lineNum)->imm));
		code += (offset & 0x1F) << 7;
